    ...
    <div class="pc-container">
        <div class="pc-content">
            ...
            <ul class="nav nav-tabs" id="ordersTabs" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="day_before_yesterday-tab" data-bs-toggle="tab"
                        data-bs-target="#day_before_yesterday" type="button" role="tab">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="day_after_tomorrow-tab" data-bs-toggle="tab"
                        data-bs-target="#day_after_tomorrow" type="button" role="tab">
                                    ...
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <div>
                                            @if ($order->orderable_type == 'App\Models\Excursion')
                                                <strong>{{ __('Category Excustion') }}</strong> -
                                                {{ $order->orderable->categoryExcursion->name[app()->getLocale()] }}<br>
                                                <strong>{{ __('Sub Category Excustion') }}</strong> -
                                                {{ $order->orderable->subcategoryExcursion->name[app()->getLocale()] }}<br>
                                            @endif
                                            <strong>{{ __('Type') }}</strong> -
                                            {{ $order->orderable->name[app()->getLocale()] }}<br>
                                            <strong>{{ __('Name User') }}</strong> - {{ $order->user->name }}<br>
                                            <strong>{{ __('Name phone') }}</strong> - {{ $order->user->phone }}<br>
                                            <strong>{{ __('Room Number') }}</strong> - {{ $order->room_number }}<br>
                                            <strong>{{ $order->order_number }}</strong> - {{ $order->type }}<br>
                                            <small>{{ __('Quantity') }}: {{ $order->quantity }}</small><br>
                                            <small>{{ __('Quantity') }}: {{ $order->quantity }}</small><br>
                                            <small>{{ __('Date') }}:
                                                {{ \Carbon\Carbon::parse($order->date)->format('d M Y') }}</small>
                                        </div>
