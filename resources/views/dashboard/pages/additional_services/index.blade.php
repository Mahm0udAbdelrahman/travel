...
@section('title', __('Additional Service'))
            ...
            <div class="page-header">
                <div class="page-block">
                    <div class="page-header-title">
                        <h5 class="mb-0 font-medium">{{ __('Additional Services') }}</h5>
                    </div>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('Admin.home') }}">{{ __('Home') }}</a></li>
                        <li class="breadcrumb-item" aria-current="page">{{ __('Additional Service') }}</li>
                    </ul>
                </div>
            </div>
                ...
                <div class="col-12 col-xl-12">
                    <div class="card">
                        ...
                        <div class="card-header flex justify-between items-center">
                            <h5>{{ __('Additional Services List') }}</h5>

                            @can('additional_services-create')
                                <a href="{{ route('Admin.additional_services.create') }}" class="btn btn-primary">
                                    <i class="fas fa-add"></i> {{ __('Add Additional Services') }}
                                </a>
                            @endcan
                        </div>
                        <!-- Table -->
                        <div class="card-body">
                            <div class="table-responsive text-center">
                                <table id="example2" class="table table-striped table-bordered">
                                    ...
                                    <form action="{{ route('Admin.additional_services.bulkDelete') }}" method="post"
                                        id="bulkDeleteForm">
                                        @csrf
                                        ...
                                        <tbody>
                                            @forelse($additional_services as $additional_service)
                                                <tr>
                                                    <td>{{ $loop->index + 1 }}</td>
                                                    <td>{{ $additional_service->name[app()->getLocale()] }}</td>
                                                    <td>{{ $additional_service->description[app()->getLocale()] }}</td>
                                                    <td>
                                                        <img src="{{ $additional_service->image }}" class="img-thumbnail"
                                                            style="width:60px;">
                                                    </td>

                                                    <td>{{ $additional_service->is_active == 1 ? 'Active' : 'Unactive' }}
                                                    </td>

                                                    <td>
                                                        <input type="checkbox" name="ids[]"
                                                            value="{{ $additional_service->id }}" class="userCheckbox">
                                                    </td>

                                                    <td>
                                                        @can('additional_services-delete')
                                                            <button type="button" class="btn btn-danger w-25 delete-user-btn"
                                                                data-id="{{ $additional_service->id }}">
                                                                <i class="far fa-trash-alt"></i>
                                                            </button>
                                                        @endcan


                                                        @can('additional_services-update')
                                                            <a href="{{ route('Admin.additional_services.edit', $additional_service) }}"
                                ...
                                <div class="mt-3" style="padding:5px;direction: ltr;">
                                    {!! $additional_services->withQueryString()->links('pagination::bootstrap-5') !!}
                                </div>
