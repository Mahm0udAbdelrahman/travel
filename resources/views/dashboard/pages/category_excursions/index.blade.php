...
@section('title', __('Category Excursions'))
            ...
            <div class="page-header">
                <div class="page-block">
                    <div class="page-header-title">
                        <h5 class="mb-0 font-medium">{{ __('Category Excursions') }}</h5>
                    </div>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('Admin.home') }}">{{ __('Home') }}</a></li>
                        <li class="breadcrumb-item" aria-current="page">{{ __('Category Excursions') }}</li>
                    </ul>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            <div class="table-responsive text-center">
                                <table id="example2" class="table table-striped table-bordered">
                                    ...
                                    <form action="{{ route('Admin.category_excursions.bulkDelete') }}" method="post" id="bulkDeleteForm">
                                        @csrf
                                        ...
                                        <tbody>
                                            @forelse($category_excursions as $category_excursion)
                                                <tr>
                                                    <td>{{ $loop->index + 1 }}</td>
                                                    <td>{{ $category_excursion->name[app()->getLocale()] }}</td>
                                                    <td>{{ $category_excursion->is_active == 1 ? 'Active' : 'Unactive' }}</td>
                                                    <td>
                                                        <img src="{{ $category_excursion->image }}" class="img-thumbnail"
                                                            style="width:60px;">
                                                    </td>
                                                    <td>
                                                        <input type="checkbox" name="ids[]" value="{{ $category_excursion->id }}"
                                                            class="userCheckbox">
                                                    </td>

                                                    <td>
                                                        @can('category_excursions-delete')
                                                            <button type="button" class="btn btn-danger w-25 delete-user-btn"
                                                                data-id="{{ $category_excursion->id }}">
                                                                <i class="far fa-trash-alt"></i>
                                                            </button>
                                                        @endcan


                                                        @can('category_excursions-update')
                                                            <a href="{{ route('Admin.category_excursions.edit', $category_excursion) }}"
                                ...
                                <div class="mt-3" style="padding:5px;direction: ltr;">
                                    {!! $category_excursions->withQueryString()->links('pagination::bootstrap-5') !!}
                                </div>
                ...
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.action = '{{ url('/admin/category_excursions') }}/' + id;
                            form.method = 'POST';
